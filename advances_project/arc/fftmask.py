...
class FFTMask(nn.Module):
    ...
    def forward(self, x, provided_group_masks=None):
        assert x.ndim == 4, "x must have 4 dimensions"
        assert x.shape[-1] == self.length, "input size must match FFT Length"
        assert x.shape[-2] == self.length, "input size must match FFT Length"

        batch_size = x.shape[0]
        device, dtype = x.device, x.dtype

        if provided_group_masks is None:
            ...
            fft_mask = provided_group_masks.to(dtype) @ self.m
        ...
        return torch.real(
            torch.fft.ifft2(full_fft_mask[:, None, :, :] * torch.fft.fft2(x))
        )
